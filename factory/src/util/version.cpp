...
{                                                                                                            
 ...
 {
        int fd=-1;
        char mainCameraName[64]  = "unknown";
        char subCameraName[64]  = "unknown";
        char lcmName[128]="unknown";
        unsigned char i=0,j=0;
        camera_open_once();
        fd = open("/sys/devices/platform/image_sensor/main_camera_name",O_RDONLY);
        if(fd!=-1)
        {
            read(fd,mainCameraName,64);
            if(mainCameraName[strlen(mainCameraName)-1]=='\n') mainCameraName[strlen(mainCameraName)-1]=0;
            close(fd);
        }
        print_verinfo(info, len,  "Camera main ", mainCameraName);

        fd = open("/sys/devices/platform/image_sensor/sub_camera_name",O_RDONLY);
        if(fd!=-1)
        {
            read(fd,subCameraName,64);
            if(subCameraName[strlen(subCameraName)-1]=='\n') subCameraName[strlen(subCameraName)-1]=0;
            close(fd);
        }
        print_verinfo(info, len,  "Camera sub  ", subCameraName);
    ...
    {// ckt guoyi add fot tpd name 2014-4-3
        int fd=-1;
    ...
    property_get("ro.ckt.version.release", val, "unknown");
...
static int create_batteryinfo(char *info, int size)
{
	int fd = -1;
        int i_per = 0;      
        char PercentName[128]="0";
        char *ptr;

        fd = open("/sys/class/power_supply/battery/capacity",O_RDONLY);
        if(fd!=-1) 
	{
           read(fd,PercentName,128);
           {
             LOGD(TAG "PercentName 111= :%s\n",  PercentName);
           }
           if(strlen(PercentName)==4)
	   {
              i_per=100;
	   }
           else if(strlen(PercentName)==3)
	   {
              i_per=(PercentName[0]-48)*10+PercentName[1]-48;
	   }
           else 
	   {
              i_per=PercentName[0]-48;
	   } 
LOGD(TAG "i_per= : %d\n",  i_per);
           close(fd);
        }
       ptr  = info;        
       ptr += sprintf(ptr, "%s: %d%s \n", uistr_info_title_battery_per, i_per, uistr_info_title_battery_tag_per); 
    return 0;
}

char* display_battery(int index, char* result, bool autoreturn)                                              
{                                                                                                            
	char *buf = NULL;                                                                                          
	struct textview vi;	 /* version info */                                                                    
	text_t vi_title;                                                                                           
	...
	init_text(&vi_title, uistr_info_title_battery_per, COLOR_YELLOW);                                                         
	init_text(&info, buf, COLOR_YELLOW);                                                                       
	init_text(&info, buf, COLOR_YELLOW);                                                                       
	...
	ui_init_textview(&vi, textview_key_handler, &vi);                                                          
	vi.set_btn(&vi, NULL, NULL, &rbtn);                                                                        
	create_batteryinfo(buf, BUFSZ);                                                                                
	LOGD(TAG "after create_battery");                                                                          
	vi.set_title(&vi, &vi_title);                                                                              
	vi.set_text(&vi, &info);                                                                                   
	vi.m_pstr = trans_verinfo(info.string, &nr_line);                                                          
	vi.m_nr_lines = g_nr_lines;                                                                                
