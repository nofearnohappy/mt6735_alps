...
#define TOLERANCE_1G            (9.3)
#define TOLERANCE_2G            (10.3)
#else
#define TOLERANCE_0G            (1.00)
...
{
#if 1
    if ((acc->evt.x  >  TOLERANCE_1G  &&  acc->evt.x  <  TOLERANCE_2G) &&
    ...
    if ((acc->evt.y  >  TOLERANCE_1G  &&  acc->evt.y  <  TOLERANCE_2G) &&
    ...
    if ((acc->evt.z  >  TOLERANCE_1G  &&  acc->evt.z  <  TOLERANCE_2G) && 
        (acc->evt.x  > -TOLERANCE_0G  &&  acc->evt.x  <  TOLERANCE_0G) &&
        (acc->evt.y  > -TOLERANCE_0G  &&  acc->evt.y  <  TOLERANCE_0G)) {
        acc->tilt = TILT_Z_POS;
        return 0;
...
static void *gsensor_update_iv_thread(void *priv)
{
    ...
    while (1) {
        ...
        len = snprintf(dat->info+len, sizeof(dat->info)-len, "%+6.3f %+6.3f %+6.3f\n%s (%s)\n%s: %s %s: %s\n%s: %s %s: %s\n%s: %s %s: %s\n", 
                 ...
                 (acc->tilt != TILT_UNKNOWN) ? (uistr_info_sensor_pass) : ("FAIL"), gsensor_pos[acc->tilt],
