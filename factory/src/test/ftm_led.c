...
//tatic bool led_thread_start = false;
...
int checkledOnline(void)
{
	int red_fd, green_fd, blue_fd, bu_fd, key_fd;
	int state=0;

	#ifdef CUST_LED_HAVE_RED
	red_fd = open(RED_LED_FILE, O_RDWR);
	if(red_fd>=0)state ++;
	close(red_fd);
	#endif
	#ifdef CUST_LED_HAVE_GREEN
	green_fd = open(GREEN_LED_FILE, O_RDWR);
	if(green_fd>=0)state ++;
	close(green_fd);
	#endif
	#ifdef CUST_LED_HAVE_BLUE
	blue_fd = open(BLUE_LED_FILE, O_RDWR);
	if(blue_fd>=0)state ++;
	close(blue_fd);
	#endif
	#ifdef CUST_LED_HAVE_BUTTON_BACKLIGHT
	bu_fd = open(BUTTON_LED_FILE, O_RDWR);
	if(bu_fd>=0)state ++;
	close(bu_fd);
	#endif
	#ifdef CUST_LED_HAVE_KEYPAD_BACKLIGHT
	key_fd = open(KEYPAD_LED_FILE, O_RDWR);
	if(key_fd>=0)state ++;
	close(key_fd);
	#endif
