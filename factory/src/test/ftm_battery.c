...
static int Get_batteryinfo(void)
{
	int fd = -1;
        int i_per = 0;      
...
static void battery_update_info(struct batteryFTM *batt, char *info)
{
	...
	if (temp != -1) {		
		...
		if (batt->charger_exist) {
            ...
            batt->current_charging = ((temp-(batt->bat_voltage))*1000)/82;
            #else
	...
	#endif
    ptr += sprintf(ptr, "%s: %d%s \n", uistr_info_title_battery_per, Get_batteryinfo(), uistr_info_title_battery_tag_per); 
    return;
}
