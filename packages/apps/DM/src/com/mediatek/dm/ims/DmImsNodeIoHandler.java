    ...
    public int read(int offset, byte[] data) throws VdmException {
        ...
        try {
            ...
            if (leafValue.equals(PCSCF_ADRESS)) {
                valueToRead = imsConfig.getProvisionedStringValue(ConfigConstants.IMS_MO_PCSCF);
            } else if (leafValue.equals(PRIVATE_USER_ID)) {
                valueToRead = imsConfig.getProvisionedStringValue(ConfigConstants.IMS_MO_IMPI);
            } else if (leafValue.equals(DOMAIN_NAME)) {
                valueToRead = imsConfig.getProvisionedStringValue(ConfigConstants.IMS_MO_DOMAIN);
            } else if (leafValue.equals(TIMER_ONE)) {
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.SIP_T1_TIMER);
            } else if (leafValue.equals(TIMER_TWO)) {
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.SIP_T2_TIMER);
            } else if (leafValue.equals(TIMER_FOUR)) {
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.SIP_TF_TIMER);
            } else if (leafValue.equals(RESOURCE_ALLOCATION_MODE)) {
                // boolean value, 1 true, 0 false.
                int value = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_RESOURCE);
                if (value == 1) {
                    valueToRead = true;
                } else {
                    valueToRead = false;
                ...
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_VOICE_E);
            } else if (leafValue.equals(SMS_IP_INDICATION)) {
                // boolean value, 1 true, 0 false.
                int value = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_SMS);
                if (value == 1) {
                    valueToRead = true;
                } else {
                    valueToRead = false;
                ...
                // boolean value, 1 true, 0 false.
                int value = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_KEEPALIVE);
                if (value == 1) {
                    valueToRead = true;
                } else {
                    valueToRead = false;
                ...
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_VOICE_U);
            } else if (leafValue.equals(IMS_VOICE_TERMINATION)) {
                // boolean value, 1 true, 0 false.
                int value = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_MOBILITY);
                if (value == 1) {
                    valueToRead = true;
                } else {
                    valueToRead = false;
                ...
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_REG_BASE);
            } else if (leafValue.equals(REG_RETRY_MAX_TIME)) {
                valueToRead = imsConfig.getProvisionedValue(ConfigConstants.IMS_MO_REG_MAX);
            }
