...
public class ImsSubNodesTests extends AndroidTestCase {
    ...
    public void testReadStringNodes() throws VdmException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            try {
                expected = imsConfig.getMasterStringValue(NODE_ID_STRING[i]);
            } catch (ImsException e) {
                e.printStackTrace();
            }
    ...
    public void testReadIntNodes() throws VdmException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int expected = 0;
            try {
                expected = imsConfig.getMasterValue(NODE_ID_INT[i]);
            } catch (ImsException e) {
                e.printStackTrace();
            }
    ...
    public void testReadBoolNodes() throws VdmException {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int expected = 0;
            try {
                expected = imsConfig.getMasterValue(NODE_ID_BOOL[i]);
            } catch (ImsException e) {
                e.printStackTrace();
            }
    ...
    public void testWriteStringNodes() throws VdmException {
        ...
        String expected = null;
        try {
            expected = imsConfig.getMasterStringValue(ConfigConstants.IMS_MO_PCSCF);
        } catch (ImsException e) {
            e.printStackTrace();
        }
