...
void FT_GetInternalVersion(char *content)
{
    ...
    META_LOG("[Meta][FT] FT_GetInternalVersion ");
    if(content == NULL)
        return ;
    ...
    META_LOG("[Meta][FT] %s = %s ", RELEASE_CKT_INTERNAL_VER_TOKEN, content);
}
...
int GetCTPVersion(char **p)
{
    ...
    META_LOG("[Meta][FT] GetCTPVersion ");
    *p=punknown;
...
void FtModCustomer::exec(Frame *pFrm)
{
	...
	switch (req->cmd.m_u1Dummy)//CKT add for ctp ver
	{
		case 1:
			GetCTPVersion(&p);
			META_LOG("[Meta][FT] Ft_Customer!	%s",p);
			strncpy(peer_buf,p,peer_buff_size-1);
			peer_buf[peer_buff_size-1]=0;
