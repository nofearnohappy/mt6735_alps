...
static void GetRandomValue(unsigned char string[6])
{
    ...
    NVRAM_LOG("Enable random generation\n");
    
    /* initialize random seed */
    srand (time(NULL));
    iRandom = rand();
    NVRAM_LOG("iRandom = [%d]", iRandom);
...
static bool is_default_bt_addr(unsigned char addr[6])
{
	if(addr[0]==0x0 && addr[1]==0x0 && addr[2]==0x46 && addr[3]==0x03 && addr[4]==0x35 && addr[5]==0x01)
		return true;
	if(addr[0]==0x0 && addr[1]==0x0 && addr[2]==0x46 && addr[3]==0x03 && addr[4]==0x21 && addr[5]==0x01)
		return true;
	return false;
}
...
int nvram_bt_default_value(unsigned char *ucNvRamData)
{
    ...
    if (chipId_ready_retry >= 10){
		...
		if(is_default_bt_addr(stBtDefault_6735_0321.addr))
		{
			GetRandomValue(stBtDefault_6735_0321.addr);
		}
                memcpy(ucNvRamData, &stBtDefault_6735_0321, rec_size);
                break;
			case 0x0335:
                /* NVRAM is MT6735 0335 default */
				if(is_default_bt_addr(stBtDefault_6735_0335.addr))
				{
					GetRandomValue(stBtDefault_6735_0335.addr);
				}
