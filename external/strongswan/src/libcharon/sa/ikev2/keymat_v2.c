...
//	DBG4(DBG_IKE, "Sk_ei secret %B", &key);
	DBG1(DBG_KEY, "Sk_ei secret %B", &key);
...
//	DBG4(DBG_IKE, "Sk_er secret %B", &key);
	DBG1(DBG_KEY, "Sk_er secret %B", &key);
...
//	DBG4(DBG_IKE, "Sk_ai secret %B", &key);
	DBG1(DBG_KEY, "Sk_ai secret %B", &key);
...
//	DBG4(DBG_IKE, "Sk_ar secret %B", &key);
	DBG1(DBG_KEY, "Sk_ar secret %B", &key);
	...
	pseudo_random_function_t rekey_function, chunk_t rekey_skd)
{
	...
	*((u_int64_t*)spi_r.ptr) = id->get_responder_spi(id);
	DBG1(DBG_KEY, "SPI_i %B", &spi_i);	
	DBG1(DBG_KEY, "SPI_r %B", &spi_r);	
