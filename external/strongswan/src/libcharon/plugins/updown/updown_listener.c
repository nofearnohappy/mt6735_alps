...
#if 0				
				ret = property_set(key, prop->pcscf[i]);
				DBG1(DBG_CHD, "property_set fail with ('%s', '%s'), ret = %d", key, prop->pcscf[i], ret);	
				if (ret != 0) {
					DBG1(DBG_CHD, "1 property_set fail with ('%s', '%s'), ret=%d", key, prop->pcscf[i], ret);	
					return ;
				}
#endif				
			}
