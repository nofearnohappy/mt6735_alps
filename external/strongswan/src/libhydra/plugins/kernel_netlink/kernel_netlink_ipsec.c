	...
	traffic_selector_t* src_ts, traffic_selector_t* dst_ts)
{
	...
	DBG0(DBG_KEY, "adding SAD entry with SPI %.8x and reqid {%u}  (mark %u/0x%08x)", ntohl(spi), reqid, mark.value, mark.mask);
	...
	switch (enc_alg)
	{
		...
		case ENCR_CAMELLIA_CCM_ICV8:
		{
			...
			DBG0(DBG_KEY, "  using encryption algorithm %N with key size %d",
				 encryption_algorithm_names, enc_alg, enc_key.len * 8);
	...
	DBG0(DBG_KEY, "  using encryption key %B", &enc_key);
	...
	if (int_alg != AUTH_UNDEFINED)
	{
		...
		DBG0(DBG_KEY, "  using integrity algorithm %N with key size %d",
			 integrity_algorithm_names, int_alg, int_key.len * 8);
		...
		DBG0(DBG_KEY, "  using integrity key %B", &int_key);
	}
