...
char * ckt_at_cmd_read_file(char * file_name) {
    int err = 0;
    ...
    do {
        err = read(fd, &buf[i], 1);
        i++;
    } while ((buf[i] != EOF) && (i < AT_BUFFER_SIZE-2));
    buf[i] = '\0';
    close(fd);
    return buf;
}

char ckt_at_cmd_write_file(char *file_name,char *buf,int len){

	int fd = open(file_name, O_RDWR, 0);
	int ret;
	if (fd == -1) 
	{   
		LOGATCI(LOG_DEBUG, "xiexijun pas_ckt_atci_handler enter with fd == -1");   
		return -1;
	}
...
ATRESPONSE_t pas_ckt_atci_handler(char* cmdline, ATOP_t opType, char* response){//xxj
    int err = 0, cmdID = 0;
    char atci_usermode[PROPERTY_VALUE_MAX];
    int usermode = 0;
    char atcmdtmp[PROPERTY_VALUE_MAX];
    char cmdbuffer[100];
    
	  LOGATCI(LOG_DEBUG, "xiexijun pas_ckt_atci_handler enter with opType: %d", opType);
    ...
    switch(opType){
    	case AT_SET_OP:
	          ...
	          err = at_tok_nextint(&cmdline, &cmdID);
    		    if (err < 0) return -1;	
    		    LOGATCI(LOG_DEBUG, "xiexijun pas_ckt_atci_handler enter with cmdID: %d", cmdID);
    		    	
            if(cmdID == 10){
               //property_set(ATCI_IN_USERMODE_PROP,"1");//20150821 xiexijun enable AT CMD in user sw version
               sprintf(cmdbuffer,"\r\nMMI CMD OK\r\n");
               writeDataToSerial(cmdbuffer,strlen(cmdbuffer));
               return AT_OK;
          	}else if(cmdID == 11){
          	   property_get("gsm.serial", atcmdtmp, "unknow");	        
	             sprintf(cmdbuffer,"SN  :%s\n",atcmdtmp);
	             writeDataToSerial(cmdbuffer,strlen(cmdbuffer));
	             return AT_OK;
          	}else if(cmdID == 12){
          	   sprintf(atcmdtmp,"0");
	        	   if(1 == ckt_at_cmd_write_file("/sys/devices/platform/battery/Ckt_AT_Test_Charging_State",atcmdtmp,1))
		           {
			             sprintf(cmdbuffer,"\r\nOK\r\n");
			             return AT_OK;
		           }
	             ...
	             sprintf(atcmdtmp,"volt:%s",ckt_at_cmd_read_file("/sys/devices/platform/battery/ckt_batt_voltage"));
               sprintf(cmdbuffer,"\r\nMMI CMD %s OK\r\n",atcmdtmp);
	             writeDataToSerial(cmdbuffer,strlen(cmdbuffer));	
	             return AT_OK;      
	          }
