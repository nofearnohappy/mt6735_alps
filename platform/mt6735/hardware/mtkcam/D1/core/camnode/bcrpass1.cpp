 ...
 ************************************************************************************************/
#define LOG_TAG "MtkCam/BCRPass1"
...
dump() const
{
    MY_LOGD2("scen(%u), rbc(%d), fps(%d)",
             muScenario, muRingBufCnt, muFrameRate);
}
 ...
 *
 ********************************************************************************/
class BCRPass1Impl : public BCRPass1
{
    private:
    ...
    public: // ctor & dtor
        BCRPass1Impl(BCRPass1InitCfg const initcfg);
        ~BCRPass1Impl();
    ...
    private:
        ...
        MBOOL postBufferToPrvCB(MUINT32 const data, IImageBuffer* const pBuf, MUINT32 const ext);

    private:
        //     init config
        BCRPass1InitCfg const       mInitCfg;
        MUINT32                     muBitDepth;
        MBOOL                       mbBCRPassToPrvcb;
        //     counter
        MUINT32                     muFrameCnt;
        ...
        sem_t                       mSemStopped;
        MUINT32                     mHwToSw[2]; //map imgo(process), rrzo, imgo(pure) to sw
        PortID                      mSwToHw[1]; //map full, resize to hw
        MUINT32                     mHwToFake[1]; //map imgo, rrzo to fake
...
BCRPass1*
BCRPass1::
createInstance(BCRPass1InitCfg const initcfg)
{
    return new BCRPass1Impl(initcfg);
}
    ...
    , mpIspSyncCtrlHw(NULL)
{
...

    {
        ...
        property_get( "debug.bcrpass1.to.prvcb", value, "0"); 
        MINT32 val = atoi(value);
        if ( val == 0 ) {
            mbBCRPassToPrvcb = MFALSE;
        } else {
            mbBCRPassToPrvcb = MTRUE;
        }
        MY_LOGD("debug.bcrpass1.to.prvcb=%d", val);
    }
}
...
onInit()
{
    ...
    if (mbBCRPassToPrvcb == MTRUE) {
        mpIspSyncCtrlHw->addPass1ReplaceBuffer(0, 0);
    }
...
MBOOL
BCRPass1Impl::
postBufferToPrvCB(MUINT32 const data, IImageBuffer* const pBuf, MUINT32 const ext)
{
    MBOOL ret;
    ImgRequest outRequest;
    MUINT32 dstdata = PASS2_PRV_DST_1;
    ...
    } else
    {
        ...
        if (!ret)
        {
            ...
            //MY_LOGD("src[%d planes] dst[%d planes]", pBuf->getPlaneCount(), outRequest.mBuffer->getPlaneCount());
            for (size_t i = 0; i < pBuf->getPlaneCount(); i++)
            {
                MUINT8 *srcBuf = (MUINT8*)(pBuf->getBufVA(i));
                MUINT8 *dstBuf = (MUINT8*)(outRequest.mBuffer->getBufVA(i));
                size_t srcSize = pBuf->getBufSizeInBytes(i);
            ...
            ret = pBufHdl->enqueBuffer(dstdata, (outRequest.mBuffer));

            if ( !ret )
            {
                MY_LOGE("enque fail: data %d, buf 0x%x", dstdata, outRequest.mBuffer);
            }
        }
    }
...
getHwPortConfig(list<HwPortConfig_t>* plPortCfg)
{
    ...
    //
    // setup port mapping
    mHwToSw[0] = PASS1_FULLRAW;
    mHwToSw[1] = PASS1_PURERAW;
    mSwToHw[0] = PORT_IMGO;
    mHwToFake[0] = 0;
    //
...
startHw(list<HwPortConfig_t> & plPortCfg)
{
    ...
    //MY_LOGE("call configFrame(0x%08x)", newMagicNum);
    if ( !configFrame(newMagicNum) ) {
        MY_LOGE("configFrame failed");
        goto lbExit;
    }
    //
    if (mbBCRPassToPrvcb == MFALSE) {
...
dequeLoop()
{
    ...
    for( iter = dequeBufInfo.mvOut.begin(); iter != dequeBufInfo.mvOut.end(); iter++ )
    {
        ...
        if (mbBCRPassToPrvcb == MTRUE) {
            ret = ret && postBufferToPrvCB(mapToNodeDataType(iter->mPortID, bIsDynamicPureRaw), iter->mBuffer, iter->mMetaData.mMagicNum_hal);
        } else {
...
querySensorInfo()
{
    ...
    pHalSensorList->querySensorStaticInfo(
            ...
            &mSensorInfo);
    //
    switch (mSensorInfo.rawSensorBit)
    {
...
#define case_Format( order_bit, mappedfmt, pFmt) \
    ...
    else if ( mSensorInfo.sensorType == SENSOR_TYPE_RAW )
    {
        switch( muBitDepth )
        {
            ...
            default:
            MY_LOGE("bitdepth not supported, 0x%x", muBitDepth);
            goto lbExit;
            break;
        }
...
configFrame(MUINT32 const magicNum)
{
    ...
    currQFrameParam.mRawOutFmt = EPipe_PURE_RAW;
