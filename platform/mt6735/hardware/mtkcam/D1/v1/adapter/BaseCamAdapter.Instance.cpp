...
#endif
#if '1'==MTKCAM_HAVE_BCR
sp<ICamAdapter> createBCRCamAdapter(String8 const& rName, int32_t const i4OpenId, sp<IParamsManager> pParamsMgr);
#endif
/******************************************************************************
*
*******************************************************************************/
...
)
{
    ...
    //
    //
    int sensorIdx = (int)IHalSensorList::get()->querySensorDevIdx(i4OpenId);
    CAM_LOGD("[ICamAdapter::%s] wwm BCR i4OpenId(%d) SensorDevIdx(%d)", __FUNCTION__,i4OpenId,sensorIdx);
    #if '1'==MTKCAM_HAVE_BCR
    if (i4OpenId == 0x01)
    {
        return createBCRCamAdapter((String8)(MtkCameraParameters::APP_MODE_NAME_BCR), i4OpenId, pParamsMgr);
    }
    #endif
