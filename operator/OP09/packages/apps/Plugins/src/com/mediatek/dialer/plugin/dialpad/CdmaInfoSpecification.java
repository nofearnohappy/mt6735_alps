...
public class CdmaInfoSpecification extends PreferenceActivity {
    ...
    private boolean mIsSvlteState = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIsSvlteState = isSvlteToGsmState();
        log("mIsSvlteState = " + mIsSvlteState);
        initInfo();
    ...
    private CDMAPhone getCDMAPhone() {
        ...
        log("getCDMAPhone find cdma phone by phone LTE_DC_PHONE_ID = " + phone);
    ...
    private Phone getBackgroundPhone() {
        return PhoneFactory.getPhone(SubscriptionManager.LTE_DC_PHONE_ID);
    }
    ...
    private void fillIMEIInfo() {
        ...
        if (phones != null && phones.length >= 2) {
            mInfoMap.put(KEY_IMEI_1, phones[0].getImei());
            if (!mIsSvlteState && (null != getBackgroundPhone())) {
                log("use background gsm phone to get imei.");
                mInfoMap.put(KEY_IMEI_1, getBackgroundPhone().getImei());
            }
            mInfoMap.put(KEY_IMEI_2, phones[1].getImei());
    ...
    private boolean isSvlteToGsmState() {
        Phone[] phones = PhoneFactory.getPhones();
        for (Phone p : phones) {
            if (p.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
                return false;
